  ...
  async function request(path, body, token, headers = {}) {
    return new Promise((resolve, reject) => {
      const req = httpRequest(root + '/bunko' + path, { method: body ? 'POST' : 'GET', headers: { Host: 'llm.lazying.art', Origin: 'https://lachlan.lazying.art', 'Content-Type': 'application/json', 'X-Bunko-Client': '1', ...(token ? { Authorization: `Bearer ${token}` } : {}), ...headers } }, response => {
        ...
        response.on('end', () => resolve({ status: response.statusCode, body: response.headers['content-type']?.startsWith('application/json') ? JSON.parse(text) : text }))
      })
      req.on('error', reject); req.end(body ? JSON.stringify(body) : undefined)
    })
  }
  async function start(platform = 'web') {
    const verifier = opaque(), started = await request('/v1/auth/start', { challenge: sha(verifier), platform })
...
test('PKCE, state replay, verifier binding, token isolation, and single-use completion', async t => {
