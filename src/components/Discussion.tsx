...
export function Discussion({ passage, excerpt, ui }: { passage: string; excerpt: string; ui: UILanguage }) {
  ...
  useEffect(() => {
    ...
    void restoreSession().catch(() => { if (!controller.signal.aborted) setError(old => old || 'offline') })
    readDiscussion(passage, 1, controller.signal).then(setThread).catch(reason => { if (!controller.signal.aborted) setError(reason instanceof DiscussionError ? reason.code : 'offline') }).finally(() => { if (!controller.signal.aborted) setLoading(false) })
  ...
  return <section className="native-discussion" aria-busy={busy}>
    ...
    {error && <p className="discussion-error" role="alert">{specificErrors[ui][error as keyof typeof specificErrors.en] ?? message}</p>}
