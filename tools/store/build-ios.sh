...
xcodebuild -project App.xcodeproj -scheme App -configuration Release -destination generic/platform=iOS -archivePath "$bunko_root/release/Bunko-1.0.0-1.xcarchive" -derivedDataPath "$bunko_root/release/DerivedData" -jobs 3 archive COMPILER_INDEX_STORE_ENABLE=NO "OTHER_CODE_SIGN_FLAGS=--keychain $bunko_kc"
xcodebuild -exportArchive -archivePath "$bunko_root/release/Bunko-1.0.0-1.xcarchive" -exportOptionsPlist ExportOptions.plist -exportPath "$bunko_root/release/export-1.0.0-1"
shasum -a 256 "$bunko_root/release/export-1.0.0-1/App.ipa"
